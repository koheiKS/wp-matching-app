...
<?php
		...
		$is_supplier = $user_role == 'サービス提供者' || $user_role == 'administrator' ? true : false;
		?>
		<div class="mt-2">
<?php
			if ($is_supplier) {
				echo do_shortcode("[display_chat_room_list supplier_id=" .$current_user_id. "]");
			} else {
				echo do_shortcode("[display_chat_room_list customer_id=" .$current_user_id. "]");
			}
?>
        	</div>
		<?php
